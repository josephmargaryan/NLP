...
class SimilarityEngine:
    ...
    def country_rule_based(self, record, df_target):
        """
        Rule-based matching for country.
        """
        df_target = df_target.copy()
        country_record = record[self.columns["country"]["source"]]
        df_target["country_match"] = df_target[self.columns["country"]["target"]].apply(
            lambda x: 100 if str(x) == str(country_record) else 0
        )
        return df_target[[self.columns["id"]["target"], "country_match"]]
    ...
    def address_embedding_similarity(self, record, df_target):
        ...
        if address_embedding_record is None:
            ...
            df_target["address_embedding_similarity"] = df_target[
                ...
                lambda x: (
                    cosine_similarity([address_embedding_record], [x])[0][0]
                    if x is not None
                    else 0
                )
            )
    ...
    def apply_similarity(self, record, df_target):
        ...
        if self.submodules.get("use_country", False):
            similarity_dfs.append(self.country_rule_based(record, df_target))
        ...
        result_df["overall_similarity"] = (
            ...
            * self.weights.get("address_similarity", 0)
            + result_df["subtype_similarity"]
            * self.weights.get("subtype_similarity", 0)
        )

        # Check for rule-based match
        result_df["rule_based_match"] = (
            (result_df["phone_match"] == 100)
            | (result_df["email_match"] == 100)
            | (result_df["country_match"] == 100)
        )

        # Add source_id and target_id for clarity
        result_df["source_id"] = record[self.columns["id"]["source"]]
        result_df["target_id"] = result_df[self.columns["id"]["target"]]

        # Apply match label logic
    ...
    def compare_single_record(self, record, df_target):
        """
        Compare one record from source to all records in the target.
        """
        return self.apply_similarity(record, df_target)

    def compare_multiple_records(self, df_source, df_target):
        """
        Compare multiple records from source to target.
        """
        results = []
...
if __name__ == "__main__":
    engine = SimilarityEngine(
        config_path="/home/jmar/matching_project/workspace2/yml_examples/engine.yml"
    )
    source = pd.read_csv(
        "/home/jmar/matching_project/workspace2/synthetic_data/Source_Data.csv"
    )
    target = pd.read_csv(
        "/home/jmar/matching_project/workspace2/synthetic_data/Target_Data.csv"
    )
