...
class SimilarityEngine:
    ...
    def phone_number_rule_based(self, record, df_target):
        ...
        Only applies if the phone number in the record is not NaN.
        """
        df_target = df_target.copy()
        phone_record = record[self.columns["phone"]["source"]]

        # Check if phone number in the source record is NaN
        if pd.isna(phone_record):
            df_target["phone_match"] = 0  # Set match score to 0 if source phone is NaN
        else:
            df_target["phone_match"] = df_target[self.columns["phone"]["target"]].apply(
                lambda x: 100 if str(x) == str(phone_record) and pd.notna(x) else 0
            )
    ...
    def email_rule_based(self, record, df_target):
        ...
        Only applies if the email in the record is not NaN.
        """
        df_target = df_target.copy()
        email_record = record[self.columns["email"]["source"]]

        # Check if email in the source record is NaN
        if pd.isna(email_record):
            df_target["email_match"] = 0  # Set match score to 0 if source email is NaN
        else:
            df_target["email_match"] = df_target[self.columns["email"]["target"]].apply(
                lambda x: 100 if str(x) == str(email_record) and pd.notna(x) else 0
            )
    ...
    def apply_similarity(self, record, df_target):
        ...
        if self.submodules.get("use_type", False):
        ...
        result_df["overall_similarity"] = 0

        if "first_name_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
                "first_name_similarity"
            ] * self.weights.get("first_name_similarity", 0)
        if "last_name_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
                "last_name_similarity"
            ] * self.weights.get("last_name_similarity", 0)
        if "subtype_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
                "subtype_similarity"
            ] * self.weights.get("subtype_similarity", 0)
        if "type_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
                "type_similarity"
            ] * self.weights.get("type_similarity", 0)
        if "address_fuzzy_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
                "address_fuzzy_similarity"
            ] * self.weights.get("address_fuzzy_similarity", 0)
        if "address_embedding_similarity" in result_df:
            result_df["overall_similarity"] += result_df[
    ...
    def compare_single_record(self, record, df_target):
        """
    ...
    def compare_multiple_records(
        self, df_source, df_target, output_filename="similarity_results.parquet"
    ):
        """
        ...
        similarity_results.to_parquet(output_path, index=False)
...
if __name__ == "__main__":
    engine = SimilarityEngine(
        config_path="/home/jmar/matching_project/similarity_engine/yml_examples/engine.yml"
    )
    target = pd.read_csv(
        "/home/jmar/matching_project/similarity_engine/synthetic_data/Source_Data.csv"
    )
    source = pd.read_csv(
        "/home/jmar/matching_project/similarity_engine/synthetic_data/Target_Data.csv"
    )
