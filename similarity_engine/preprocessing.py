...
class Preprocessing:
    ...
    def standardize_last_name(self, df, last_name_column):
        """
        Standardizes the last name column by extracting the last name and converting it to lowercase.

        Params:
        - df (DataFrame): The dataframe to process.
        - last_name_column (str): The name of the column containing full last names.

        Returns:
        - df (DataFrame): The dataframe with the standardized last name column.
        """
        # Check if the column exists in the dataframe
        if last_name_column not in df.columns:
            print(f"Column '{last_name_column}' not found in the dataframe.")
            return df

        # Apply the transformation to extract the last name and convert it to lowercase
        df["standardized_last_name"] = df[last_name_column].apply(
    ...
    def process_data(self):
        ...
        if self.config.get("source_address_columns"):
            source_address_columns = self.config["source_address_columns"]
        ...
        if self.config.get("source_df_last_name"):
            source_last_name_column = self.config["source_df_last_name"]
            source_df = self.standardize_last_name(source_df, source_last_name_column)

        if self.config.get("target_df_last_name"):
            target_last_name_column = self.config["target_df_last_name"]
            target_df = self.standardize_last_name(target_df, target_last_name_column)
...
if __name__ == "__main__":
    source_df, target_df, merged_df = Preprocessing(
