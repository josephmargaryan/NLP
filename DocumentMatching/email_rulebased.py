...
def email_rule_based_single(record, df_target):
    """
    Rule-based matching of emails for a single record against all rows in the target dataframe.

    Params:
    - record (Series): A single record containing the 'EMAIL' field.
    - df_target (DataFrame): Target dataframe containing the 'email_1__v' field.

    Returns:
    - result_df (DataFrame): DataFrame containing email match scores.
    """
    email_record = str(record["EMAIL"]).strip().lower()

    df_target = df_target.copy()
    df_target["email_1__v"] = df_target["email_1__v"].apply(
        lambda x: str(x).strip().lower()
    )

    df_target["email_match"] = df_target.apply(
        lambda row: 100 if row["email_1__v"] == email_record else 0, axis=1
    )

    return df_target[["vid__v", "email_1__v", "email_match"]]
